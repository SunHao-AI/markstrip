...
- pragma 指令在 selective 模式下"该文件应转 full 模式"（全量删注释保留代码）

典型场景：整个文件是交付时不应包含任何注释的"clean zone"，用 pragma 一次性指令，无需逐行 `@internal`。
...
- 扫描器（`scan_file_pragma`）扫描全部行，任意位置出现 `# markstrip: full` 即生效
...
| Python 块定界 | `# @internal-start` ... `# @internal-end` | 块区域 | 删除两个定界行及其间的所有注释（纯注释整行删除，行尾注释仅删注释部分），保留代码行 |
